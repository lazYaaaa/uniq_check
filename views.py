...
class SiteDetailView(LoginRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('make-pdf') is not None:
            ...
            path_to_wkhtmltopdf = r'C:/Users/al.foll/Documents/PythonProject/backup/backupofsiteno2/wkhtmltopdf/bin/wkhtmltopdf.exe'  # Путь к wkhtmltopdf
            config = pdfkit.configuration(wkhtmltopdf=path_to_wkhtmltopdf)
            context = {"images": base_64_images, "site": site}
            report_page = get_template('main/report.html').render(context)
            pdf = pdfkit.from_string(report_page, False, configuration=config, options={"enable-local-file-access": ""})
        ...
        if request.POST.get('site-report') is not None:
            ...
            reps[last_time] = str(int(reps[last_time]) + 1)
            site.save()
            return redirect(f'/site/{site.id}/')
...
class SiteListView(ListView):
    ...
    queryset = Sites.objects.order_by('id')
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        with open('C:/Users/al.foll/Documents/PythonProject/backup/backupofsiteno2/parsed_data/sites.json', 'r') as f:
            json_data = json.load(f)

        # Преобразуем данные в удобный вид для цикла в шаблоне
        sites_list = []
        for index, (site_name, site_info) in enumerate(json_data.items()):
            sites_list.append({
                'name': site_name,
                'url': site_info['site'],
                'logo': site_info['logo_file'],
                'id': index + 1
            })
        context['sites_from_json'] = sites_list
...
class UserSiteDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('make-pdf') is not None:
            ...
            base_64_images = graph_form.cleaned_data['graph']
            base_64_images = base_64_images.split('data:image/png;base64,')
            config = pdfkit.configuration(wkhtmltopdf="C:/Users/al.foll/Documents/PythonProject/backup/backupofsiteno2/wkhtmltopdf/bin/wkhtmltopdf.exe")
            context = {"images": base_64_images, "site": site}
            report_page = get_template('main/report.html').render(context)
            pdf = pdfkit.from_string(report_page, False, options={"enable-local-file-access": ""}, configuration=config)
